        ...
        try{
            ...
            if($request->hasFile('file')){
                    ...
                    if($getCandidateDocuments->json() != ''){
                        ...
                        for($i=0; $i<$count;$i++){
                            if($getCandidateDocumentType[4]['document_type_id'] == $getCandidateDocuments[$i]['document_title']){
                        ...
                        if($document_id != ''){
                            ...
                            $setCandidateDocuments = Http::asForm()->post($url, [
                                    ...
                                    'document_title' => $getCandidateDocumentType[4]['document_type_id'],
                    ...
                    if($setCandidateDocuments->json() != ''){
                        return redirect()->back()->with('success','Passbook is uploaded successfully!');
                    }
                    else{
                        return redirect()->back()->with('error','Failed to upload Passbook! Please try again');
                    }
    ...
    public function printPassbook()
    {
        try{
            ...
            if($document_path != ''){
                ...
                return back()->with('error','No Passbook is available to print');
            }
        }catch(Exception $e){
            echo '<html><head><title>Error 404</title><meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
